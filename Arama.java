...
public class Arama extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jTable1.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "İndex numarası", "Ürün", "Adet", "Birim Fiyatı"
            }
        ) {
   ...
   // arama butonu
        try{
            if( jTextField1.getText().equals(""))
                JOptionPane.showMessageDialog(null, "Lutfen urun giriniz: ", "Bekleyiniz..", JOptionPane.ERROR_MESSAGE);
            else
            {
                for( int r=0; r<100; r++)  
                    for( int c=0; c<4; c++) 
               ...
               BufferedReader dosyaOku= new BufferedReader( new FileReader("items.txt"));
               String[] satir= new String[100];
               String arama="", cikti="", hedef="";
                
                boolean ayniMi= false, bayrak=false;

                int x=0, satir_=0;
                while( (satir[x]= dosyaOku.readLine()) != null)//reading items.txt; asigning to array[] line
                    x++;

                dosyaOku.close();

                arama= jTextField1.getText();

                for( int k=0; satir[k] != null; k++){
                    ayniMi= false;
                    hedef="";
                    for( int j=0; j < arama.length(); j++)
                        hedef+= satir[k].charAt(j);

                    if( arama.equals(hedef))
                        ayniMi=true;

                    if( ayniMi){
                        String[] temp= satir[k].split("\t");
                        jTable1.setValueAt((1000+k+1), satir_, 0);
                        for( int i=1; i<4; i++)
                            jTable1.setValueAt(temp[i-1], satir_,i);
                        satir_++;
                        bayrak=true;
                    }
               } 
                ...
                if( !bayrak)
                    JOptionPane.showMessageDialog(null, "Urun(ler) bulunamadi !", "HATA !", JOptionPane.ERROR_MESSAGE);
